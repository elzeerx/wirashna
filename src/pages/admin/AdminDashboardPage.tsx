...
import { BarChart3, Users, CalendarDays, DollarSign, AlertTriangle, FileText, UserPlus, Plus, PencilRuler } from "lucide-react";
...
const AdminDashboardPage = () => {
  ...
  if (isLoading) {
    return <AdminDashboardLayout isLoading={true} />;
  }
  ...
  const completionRate = workshops.length > 0 ? Math.round(totalParticipants / workshops.reduce((sum, w) => sum + w.total_seats, 0) * 100) : 0;
  ...
  return <AdminDashboardLayout>
      <div className="grid grid-cols-1 gap-8 p-6 md:grid-cols-3 lg:grid-cols-3 xl:grid-cols-3 px-[8px] py-0 mx-0">
        ...
        <Suspense fallback={<SkeletonLoader count={1} className="h-64" />}>
          <div className="md:col-span-1 lg:col-span-1 xl:col-span-1">
            <RecentActivities title="النشاطات الأخيرة" activities={recentActivities} onViewAll={() => navigate('/admin')} />
          </div>
        </Suspense>
      </div>
